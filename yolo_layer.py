...
from utils import bbox_iou, multi_bbox_ious, multi_max_bbox_ious, convert2cpu

class YoloLayer(nn.Module):
    def __init__(self, anchor_mask=[], num_classes=0, anchors=[1.0], num_anchors=1, num_props=4, use_cuda=None):
        super(YoloLayer, self).__init__()
        use_cuda = torch.cuda.is_available() and (True if use_cuda is None else use_cuda)
        self.device = torch.device("cuda" if use_cuda else "cpu")
        self.csv_write = open('./visual_loss.csv', 'wb')
    ...
    def get_mask_boxes(self, output):
        ...
        # print(self.stride, self.anchor_mask)
    ...
    def build_targets(self, pred_boxes, target, anchors, nA, nH, nW):
        ...
        prop_mask = torch.zeros(nB, nA, nH, nW)
        cls_mask   = torch.zeros(nB, nA, nH, nW)
        tcoord     = torch.zeros( 4, nB, nA, nH, nW)
        tprop     = torch.zeros( self.num_props, nB, nA, nH, nW)
        ...
        for b in range(nB):
            ...
            tbox = target[b].view(-1, 5 + self.num_props).to("cpu")
            for t in range(200):
                if tbox[t][1] == 0:
                    break
            ...
            for t in range(200):
                ...
                for tp in range(self.num_props):
                    tprop[tp][b][best_n][gj][gi] = tbox[t][5 + tp]
        ...
        return nGT, nRecall, nRecall75, coord_mask, conf_mask, cls_mask, prop_mask, tcoord, tconf, tcls, tprop
    ...
    def forward(self, output, target):
        ...
        prop_grid = torch.linspace(5+nC,5+nC+nF-1,nF).long().to(self.device)
        ...
        prop  = prop.view(nB*nA, nF, nH*nW).transpose(1,2).contiguous().view(cls_anchor_dim, nF)
        ...
        n = 0
        if n > 0:
            loss_coord = nn.MSELoss(size_average=False)(coord * coord_mask, tcoord * coord_mask) / n
            loss_coord = loss_coord ** 2
            loss_conf = nn.MSELoss(size_average=False)(conf * conf_mask, tconf * conf_mask) / n
        ...
        if loss_coord > 5:
            ...
            # else:
            #     coe = [0.4, 0.1, 0.1, 0.4]
        loss = coe[0] * loss_coord + coe[1] * loss_conf + coe[2] * loss_cls + coe[3] * loss_prop
        ...
        if False:
            ...
            print('             total : %f' % (t4 - t0))
        print('[%s] %d: Layer(%03d) nGT %3d, nRC %3d, nRC75 %3d, nPP %3d, |loss: box =%6.3f, conf =%6.3f, class =%6.3f, prop =%6.3f, |total %7.3f|, num =%d'
                % (time.strftime('%H:%M:%S %d,%b.', time.localtime()), self.seen, self.nth_layer, nGT, nRecall, nRecall75, nProposals, loss_coord, loss_conf, loss_cls, loss_prop, loss, n))
